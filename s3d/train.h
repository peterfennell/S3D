...
int calculate_N(ifstream & datafile)
{
...
void create_Xj_info(int & j, vector<map<double,unsigned int> > & cumcount, vector<map<double,double> > & cumsum, set<double> & range, ifstream & datafile, vector<unsigned int> & groups, int & G, int y_col, int N, int start_skip_rows, int end_skip_rows)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(i == start_skip_rows)
        {
            // skip the next rows until end_skip_rows
